...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${apiUrl}/v1/commission/simulation/return/items`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      throw new Error(data.message || "Failed to start items return simulation");
    }

    console.log("Items return simulation started:", data);

    return NextResponse.json({
      success: true,
      message: "Items return simulation started successfully",
      data,
    });
  } catch (error) {
    console.error("Error starting items return simulation:", error);
    return NextResponse.json(
      {
